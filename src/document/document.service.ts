...
import { Injectable, Post, UploadedFile, UseInterceptors, Module } from '@nestjs/common';
...
export class DocumentService extends TypeOrmCrudService<Documents> {
  constructor(@InjectRepository(Documents) repo, private configService: ConfigService) {
    super(repo);
  }
  ...
  deleteDocument(docId: number) {
    let doc = this.getDocument(docId).then((val) => {
      return this.repo.delete(val).then(res => {
  ...
  async getDocuments(oid: number, owner: DocumentOwner): Promise<Documents[]> {
    let documenst = await this.repo.find({ where: { documentOwnerId: oid, documentOwner: owner } });
